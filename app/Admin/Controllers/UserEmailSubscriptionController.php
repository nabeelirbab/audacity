...
class UserEmailSubscriptionController extends AdminController
{
    ...
    protected function form()
    {
        return new Form(new UserEmailSubscription(), function (Form $form) {
            $options = User::whereManagerId(Admin::user()->id)->selectRaw('CONCAT (name, " - ", email) as c, id')->pluck('c', 'id');
            $me = Admin::user();
            ...
            $form->select('email_subscription_id', 'Subscription')
                ->options(EmailSubscription::whereManagerId(Admin::user()->id)->pluck('title', 'id'))->required();

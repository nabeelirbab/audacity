    ...
    public function handle()
    {
        try {
            ...
            $accounts = Account::with('manager:id,username')
                ...
                ->where('copier_type', CopierType::FOLLOWER)
                ->get(['account_number', 'broker_server_name','api_server_ip','manager_id'])
                ->toArray();

            if(count($accounts) > 0)
                $this->error('Slave Accounts with investor passwords', ['accounts' => $accounts]);
            ...
            $accounts = Account::with('manager:id,username')
                ...
                ->toArray();
            if(count($accounts) > 0)
                $this->error('Slave Accounts with fake pairs name and empty broker suffix', ['accounts' => $accounts]);
        } catch (\Exception $e) {

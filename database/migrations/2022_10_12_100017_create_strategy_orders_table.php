...
class CreateStrategyOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('strategy_orders', function (Blueprint $table) {
            ...
            $table->double('lots', 5, 2)->nullable();
            ...
            $table->timestamps();
            $table->unsignedInteger('strategy_id')->nullable();
            
            $table->unique(['ticket', 'account_number'], 'index_system_orders_on_system_id_and_ticket_and_master_id');
            $table->foreign('strategy_id', 'strategy_orders_ibfk_1')->references('id')->on('strategies')->onDelete('cascade');
        });
    }
